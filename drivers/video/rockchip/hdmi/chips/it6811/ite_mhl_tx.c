...
{
    TX_DEBUG_PRINT("MhlTx:GetNextCBusTransaction: %s depth: %d  head: %d  tail: %d\n", pszFunction, (int)QUEUE_DEPTH(CBusQueue), (int)CBusQueue.head,(int)CBusQueue.tail);
...
{
    bool_t retVal;
    TX_DEBUG_PRINT("MhlTx: PutNextCBusTransaction %02X %02X %02X depth:%d head: %d tail:%d\n",(int)pReq->command,(int)((MHL_MSC_MSG == pReq->command)?pReq->payload_u.msgData[0]:pReq->offsetData),(int)((MHL_MSC_MSG == pReq->command)?pReq->payload_u.msgData[1]:pReq->payload_u.msgData[0]),(int)QUEUE_DEPTH(CBusQueue),(int)CBusQueue.head,(int)CBusQueue.tail);
...
static bool_t ITE6811MhlTxSetDCapRdy( void );
static	bool_t 		ITE6811MhlTxRapkSend( uint8_t rapkErrCode );
static	void		MhlTxResetStates( void );
static	bool_t		MhlTxSendMscMsg ( uint8_t command, uint8_t cmdData );
static void ITE6811MhlTxRefreshPeerDevCapEntries(void);
static void MhlTxDriveStates( void );
extern PLACE_IN_CODE_SEG uint8_t rcpSupportTable [];

bool_t MhlTxCBusBusy(void)
{
    return ((QUEUE_DEPTH(CBusQueue) > 0)||ITE6811MhlTxDrvCBusBusy() || mhlTxConfig.cbusReferenceCount)?true:false;
}
...
{
	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxTmdsEnable\n");
        ...
        if (MHL_HPD & mhlTxConfig.mhlHpdRSENflags) {
            ...
            if (QualifyPathEnable()) {
            	if (RAP_CONTENT_ON & mhlTxConfig.rapFlags) {
                    ...
                    ITE6811MhlTxDrvTmdsControl(true);
                }
            }
        }
    }
}
static bool_t ITE6811MhlTxSetInt( uint8_t regToWrite,uint8_t  mask, uint8_t priorityLevel )
{
	cbus_req_t	req;
    bool_t retVal;
    req.retryCount  = 2;
...
static bool_t ITE6811MhlTxDoWriteBurst( uint8_t startReg, uint8_t *pData,uint8_t length )
{
    if (FLAGS_WRITE_BURST_PENDING & mhlTxConfig.miscFlags) {
        cbus_req_t	req;
        bool_t retVal;
    	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxDoWriteBurst startReg:%d length:%d\n",(int)startReg,(int)length);
        req.retryCount  = 1;
        ...
        retVal = PutPriorityCBusTransaction(&req);
        ClrMiscFlag(ITE6811MhlTxDoWriteBurst, FLAGS_WRITE_BURST_PENDING)
        return retVal;
    }
    return false;
}
ScratchPadStatus_e ITE6811MhlTxRequestWriteBurst(uint8_t startReg, uint8_t length, uint8_t *pData)
{
	...
	if (!(MHL_FEATURE_SP_SUPPORT & mhlTxConfig.mscFeatureFlag)) {
		TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxRequestWriteBurst failed SCRATCHPAD_NOT_SUPPORTED\n");
		retVal= SCRATCHPAD_NOT_SUPPORTED;
	} else {
		if ((FLAGS_SCRATCHPAD_BUSY & mhlTxConfig.miscFlags) || MhlTxCBusBusy()) {
			TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxRequestWriteBurst failed FLAGS_SCRATCHPAD_BUSY \n");
		} else {
			bool_t temp;
			uint8_t i,reg;
			TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxRequestWriteBurst, request sent\n");
			for (i = 0,reg=startReg; (i < length) && (reg < SCRATCHPAD_SIZE); ++i,++reg) {
				mhlTxConfig.localScratchPad[reg]=pData[i];
			}
			temp =  ITE6811MhlTxSetInt(MHL_RCHANGE_INT,MHL_INT_REQ_WRT, 1);
...
void ITE6811MhlTxInitialize(uint8_t pollIntervalMs )
{
	...
	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxInitialize\n");
	CBusQueue.head = 0;
	CBusQueue.tail = 0;
	mhlTxConfig.pollIntervalMs  = pollIntervalMs;
    TX_DEBUG_PRINT("MhlTx: HPD: %d RSEN: %d\n",(int)((mhlTxConfig.mhlHpdRSENflags & MHL_HPD)?1:0),(int)((mhlTxConfig.mhlHpdRSENflags & MHL_RSEN)?1:0));
	MhlTxResetStates( );
    TX_DEBUG_PRINT("MhlTx: HPD: %d RSEN: %d\n",(mhlTxConfig.mhlHpdRSENflags & MHL_HPD)?1:0,(mhlTxConfig.mhlHpdRSENflags & MHL_RSEN)?1:0);
	ITE6811MhlTxChipInitialize ();
		...
		switch (mhlTxConfig.mscMsgSubCommand) {
			case	MHL_MSC_MSG_RAP:
				if (MHL_RAP_CONTENT_ON == mhlTxConfig.mscMsgData) {
                    ...
                    ITE6811MhlTxRapkSend(MHL_MSC_MSG_RAP_NO_ERROR);
				} else if( MHL_RAP_CONTENT_OFF == mhlTxConfig.mscMsgData) {
					mhlTxConfig.rapFlags &= ~RAP_CONTENT_ON;
		            TX_DEBUG_PRINT("RAP CONTENT_OFF\n");
					ITE6811MhlTxDrvTmdsControl( false );
                    ITE6811MhlTxRapkSend(MHL_MSC_MSG_RAP_NO_ERROR);
				} else if(MHL_RAP_CMD_POLL == mhlTxConfig.mscMsgData) {
                    ITE6811MhlTxRapkSend(MHL_MSC_MSG_RAP_UNSUPPORTED_ACT_CODE);
                } else {
                    ITE6811MhlTxRapkSend(MHL_MSC_MSG_RAP_UNRECOGNIZED_ACT_CODE);
                }
				break;
			...
			case	MHL_MSC_MSG_RCP:
				if(MHL_LOGICAL_DEVICE_MAP & rcpSupportTable [mhlTxConfig.mscMsgData & 0x7F] ) {
					...
					ITE6811MhlTxRcpeSend( RCPE_INEEFECTIVE_KEY_CODE );
				}
				break;
    ...
    if (QUEUE_DEPTH(CBusQueue) > 0) {
        ...
        if (!ITE6811MhlTxDrvCBusBusy()) {
            ...
            if (reQueueRequest) {
                ...
                IncrementCBusReferenceCount(MhlTxDriveStates)
                ITE6811MhlTxDrvSendCbusCommand( pReq  );
            }
        }
    }
}
static void ExamineLocalAndPeerVidLinkMode( void )
{
	...
	if (MHL_DEV_VID_LINK_SUPP_PPIXEL & mhlTxConfig.aucDevCapCache[DEVCAP_OFFSET_VID_LINK_MODE]) {
 	...
 	ITEMhlTxDrvSetClkMode(mhlTxConfig.linkMode & MHL_STATUS_CLK_MODE_MASK);
}
#define FLAG_OR_NOT(x) (FLAGS_HAVE_##x & mhlTxConfig.miscFlags)?#x:""
#define SENT_OR_NOT(x) (FLAGS_SENT_##x & mhlTxConfig.miscFlags)?#x:""
void	ITE6811MhlTxMscCommandDone( uint8_t data1 )
{
	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone. data1 = %02X\n", (int) data1);
    DecrementCBusReferenceCount(ITE6811MhlTxMscCommandDone)
    if (MHL_READ_DEVCAP == mhlTxConfig.mscLastCommand) {
		if (mhlTxConfig.mscLastOffset < sizeof(mhlTxConfig.aucDevCapCache)) {
			...
			TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone peer DEV_CAP[0x%02x]:0x%02x index:0x%02x\n",(int)(mhlTxConfig.mscLastOffset),(int)data1,(int)(mhlTxConfig.ucDevCapCacheIndex));
			...
			if (++mhlTxConfig.ucDevCapCacheIndex < sizeof(mhlTxConfig.aucDevCapCache)) {
				ITE6811MhlTxReadDevcap( mhlTxConfig.ucDevCapCacheIndex );
			} else {
        ...
        if (MHL_STATUS_REG_CONNECTED_RDY == mhlTxConfig.mscLastOffset) {
            if (MHL_STATUS_DCAP_RDY & mhlTxConfig.mscLastData) {
            	...
            	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone FLAGS_SENT_DCAP_RDY\n");
                ITE6811MhlTxSetInt(MHL_RCHANGE_INT,MHL_INT_DCAP_CHG, 0); 
            }
            ...
            if ( MHL_STATUS_PATH_ENABLED & mhlTxConfig.mscLastData) {
            	...
            	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone FLAGS_SENT_PATH_EN\n");
   	    	}
		}
       	mhlTxConfig.mscLastCommand = 0;
       	mhlTxConfig.mscLastOffset  = 0;
    	...
    	if(MHL_MSC_MSG_RCPE == mhlTxConfig.mscMsgLastCommand) {
			ITE6811MhlTxRcpkSend(mhlTxConfig.mscSaveRcpKeyCode);
    	} else {
    	    TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone default\n" "\tmscLastCommand: 0x%02X \n" "\tmscMsgLastCommand: 0x%02X mscMsgLastData: 0x%02X\n" "\tcbusReferenceCount: %d\n",(int)mhlTxConfig.mscLastCommand,(int)mhlTxConfig.mscMsgLastCommand,(int)mhlTxConfig.mscMsgLastData,(int)mhlTxConfig.cbusReferenceCount);
        }
        mhlTxConfig.mscLastCommand = 0;
    } else if (MHL_WRITE_BURST == mhlTxConfig.mscLastCommand) {
        TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone MHL_WRITE_BURST\n");
        mhlTxConfig.mscLastCommand = 0;
        mhlTxConfig.mscLastOffset  = 0;
        mhlTxConfig.mscLastData    = 0;
        ITE6811MhlTxSetInt( MHL_RCHANGE_INT,MHL_INT_DSCR_CHG,0 );
    } else if (MHL_SET_INT == mhlTxConfig.mscLastCommand) {
        TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone MHL_SET_INT\n");
        if (MHL_RCHANGE_INT == mhlTxConfig.mscLastOffset) {
        	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone MHL_RCHANGE_INT\n");
            if (MHL_INT_DSCR_CHG == mhlTxConfig.mscLastData) {
                DecrementCBusReferenceCount(ITE6811MhlTxMscCommandDone)  
                TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone MHL_INT_DSCR_CHG\n");
                ClrMiscFlag(ITE6811MhlTxMscCommandDone, FLAGS_SCRATCHPAD_BUSY)
            }
        }
        mhlTxConfig.mscLastCommand = 0;
        mhlTxConfig.mscLastOffset  = 0;
    	...
    	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone default\n" "\tmscLastCommand: 0x%02X mscLastOffset: 0x%02X\n" "\tcbusReferenceCount: %d\n",(int)mhlTxConfig.mscLastCommand,(int)mhlTxConfig.mscLastOffset,(int)mhlTxConfig.cbusReferenceCount);
    }

    if (!(FLAGS_RCP_READY & mhlTxConfig.miscFlags)) {
    	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscCommandDone. have(%s %s) sent(%s %s)\n", FLAG_OR_NOT(DEV_CATEGORY), FLAG_OR_NOT(DEV_FEATURE_FLAGS), SENT_OR_NOT(PATH_EN), SENT_OR_NOT(DCAP_RDY));
...
void	ITE6811MhlTxMscWriteBurstDone( uint8_t data1 )
{
#define WRITE_BURST_TEST_SIZE 16
	uint8_t temp[WRITE_BURST_TEST_SIZE];
	uint8_t i;

    TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxMscWriteBurstDone(%02X) \"",(int)data1);
    ITE6811MhlTxDrvGetScratchPad(0,temp,WRITE_BURST_TEST_SIZE);
...
void	ITE6811MhlTxGotMhlMscMsg( uint8_t subCommand, uint8_t cmdData )
{
	mhlTxConfig.mscMsgArrived		= true;
	mhlTxConfig.mscMsgSubCommand	= subCommand;
	mhlTxConfig.mscMsgData			= cmdData;
}
void	ITE6811MhlTxGotMhlIntr( uint8_t intr_0, uint8_t intr_1 )
{
	...
	if(MHL_INT_DCAP_CHG & intr_0) {
        if (MHL_STATUS_DCAP_RDY & mhlTxConfig.status_0) {
			ITE6811MhlTxRefreshPeerDevCapEntries();
		}
	}

	if( MHL_INT_DSCR_CHG & intr_0) {
        ITE6811MhlTxDrvGetScratchPad(0,mhlTxConfig.localScratchPad,sizeof(mhlTxConfig.localScratchPad));
        ClrMiscFlag(ITE6811MhlTxGotMhlIntr, FLAGS_SCRATCHPAD_BUSY)
        AppNotifyMhlEvent(MHL_TX_EVENT_DSCR_CHG,0);
    }

	if( MHL_INT_REQ_WRT  & intr_0) {
        if (FLAGS_SCRATCHPAD_BUSY & mhlTxConfig.miscFlags) {
            ITE6811MhlTxSetInt( MHL_RCHANGE_INT, MHL_INT_GRT_WRT,1);
        } else {
            SetMiscFlag(ITE6811MhlTxGotMhlIntr, FLAGS_SCRATCHPAD_BUSY)
            ITE6811MhlTxSetInt( MHL_RCHANGE_INT, MHL_INT_GRT_WRT,0);
        }
    }
	...
	if( MHL_INT_GRT_WRT  & intr_0) {
        ...
        ITE6811MhlTxDoWriteBurst(0x40, mhlTxConfig.localScratchPad, length);
    }

	if(MHL_INT_EDID_CHG & intr_1) {
		ITE6811MhlTxDrvNotifyEdidChange();
	}
}


void	ITE6811MhlTxGotMhlStatus( uint8_t status_0, uint8_t status_1 )
{
	...
	if(MHL_STATUS_PATH_ENABLED & StatusChangeBitMask1) {
        ...
        if(MHL_STATUS_PATH_ENABLED & status_1) {
            ITE6811MhlTxSetPathEn();
        } else {
            ITE6811MhlTxClrPathEn();
        }
    }
}
bool_t ITE6811MhlTxRcpSend( uint8_t rcpKeyCode )
{
	...
	{
    	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxRcpSend failed\n");
		retVal=false;
	}
	retVal=MhlTxSendMscMsg ( MHL_MSC_MSG_RCP, rcpKeyCode );
    if(retVal) {
    	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxRcpSend\n");
        IncrementCBusReferenceCount(ITE6811MhlTxRcpSend)
		MhlTxDriveStates();
	}
    return retVal;
}
...
static bool_t ITE6811MhlTxSetStatus( uint8_t regToWrite, uint8_t value )
{
	cbus_req_t	req;
    bool_t retVal;
    req.retryCount  = 2;
    ...
    TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxSetStatus\n");
    retVal = PutNextCBusTransaction(&req);
    return retVal;
}
static bool_t ITE6811MhlTxSetDCapRdy( void )
{
    ...
    return ITE6811MhlTxSetStatus( MHL_STATUS_REG_CONNECTED_RDY, mhlTxConfig.connectedReady);
}
static bool_t ITE6811MhlTxSendLinkMode(void)
{
    return ITE6811MhlTxSetStatus( MHL_STATUS_REG_LINK_MODE, mhlTxConfig.linkMode);
}
bool_t ITE6811MhlTxSetPathEn(void )
{
	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxSetPathEn\n");
    ITE6811MhlTxTmdsEnable();
...
{
	TX_DEBUG_PRINT("MhlTx: ITE6811MhlTxClrPathEn\n");
    ITE6811MhlTxDrvTmdsControl( false );
    mhlTxConfig.linkMode &= ~MHL_STATUS_PATH_ENABLED;    
    return ITE6811MhlTxSetStatus( MHL_STATUS_REG_LINK_MODE, mhlTxConfig.linkMode);
}
...
{
	cbus_req_t	req;
	TX_DEBUG_PRINT("MhlTx:ITE6811MhlTxReadDevcap\n");
    req.retryCount  = 2;
...
void ITE6811MhlTxNotifyConnection( bool_t mhlConnected )
{
	mhlTxConfig.mhlConnectionEvent = true;
	TX_DEBUG_PRINT("MhlTx: ITE6811MhlTxNotifyConnection MSC_STATE_IDLE %01X\n", (int) mhlConnected);
...
void	ITE6811MhlTxNotifyDsHpdChange( uint8_t dsHpdStatus )
{
	TX_DEBUG_PRINT("MhlTx: ITE6811MhlTxNotifyDsHpdChange dsHpdStatus=%d\n", (int) dsHpdStatus);
	...
	if (0 == dsHpdStatus) {
		...
		ITE6811DrvMhlTxReadEdid();

		iteHdmiTx_VideoSel(prefertiming);//setting the VIC and AVI info
