#define ITE6811_HAL_LINUX_INIT_C
#include <linux/i2c.h>
#include "ite_hal.h"
...
{
	if (!(gHalInitedFlag)) {
		ITE6811_DEBUG_PRINT("Error: Hal layer not currently initialize!\n");
...
halReturn_t HalInit(void)
{
	...
	if (gHalInitedFlag)
	{
		ITE6811_DEBUG_PRINT("Error: Hal layer already inited!\n");
...
halReturn_t HalAcquireIsrLock()
{
	...
	if (status != 0)
	{
		ITE6811_DEBUG_PRINT("HalAcquireIsrLock failed to acquire lock\n");
