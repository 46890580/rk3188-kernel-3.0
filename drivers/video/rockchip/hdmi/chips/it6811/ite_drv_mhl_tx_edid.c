...
uint8_t ParseEDID (uint8_t *pEdid, uint8_t *numExt)
{
	...
	if (!DoEDID_Checksum(pEdid)) {
		TX_EDID_PRINT("EDID -> Checksum Error\n");
...
uint8_t ITE6811DrvMhlTxReadEdid (void)
{
	...
	if (GetDDC_Access(&reg_val)) {
		...
		if (Result != EDID_OK) {
			if (Result == EDID_NO_861_EXTENSIONS) {
				...
				TX_EDID_PRINT("EDID -> No 861 Extensions, NOT HDMI Sink device\n");
			} else {
				TX_EDID_PRINT("EDID -> Parse FAILED\n");
			...
			TX_EDID_PRINT("EDID -> Parse OK\n");
			Result = Parse861Extensions(NumOfExtensions);		
			if (Result != EDID_OK) {
				TX_EDID_PRINT("EDID -> Extension Parse FAILED\n");
				EDID_Data.HDMI_Sink = true;
                EDID_Data.YCbCr_4_4_4 = false;
                EDID_Data.YCbCr_4_2_2 =false;
                EDID_Data.CEC_A_B = 0x00;
		...
		if (!ReleaseDDC(reg_val)) {
			TX_EDID_PRINT("EDID -> DDC bus release failed\n");
			return EDID_DDC_BUS_RELEASE_FAILURE;
		}

		TX_EDID_PRINT("EDID_Data.HDMI_Sink = %d\n", (int)EDID_Data.HDMI_Sink);
		TX_EDID_PRINT("EDID_Data.YCbCr_4_4_4 = %d\n", (int)EDID_Data.YCbCr_4_4_4);
		TX_EDID_PRINT("EDID_Data.YCbCr_4_2_2 = %d\n", (int)EDID_Data.YCbCr_4_2_2);
		TX_EDID_PRINT("EDID_Data.CEC_A_B = 0x%x\n", (int)EDID_Data.CEC_A_B);
		TX_EDID_PRINT("EDID_Data.CEC_C_D = 0x%x\n", (int)EDID_Data.CEC_C_D);
	} else {
		TX_EDID_PRINT("EDID -> DDC bus request failed\n");
        EDID_Data.HDMI_Sink = true;
        EDID_Data.YCbCr_4_4_4 = false;
        EDID_Data.YCbCr_4_2_2 =false;
        EDID_Data.CEC_A_B = 0x00;
...
static uint8_t Parse861Extensions(uint8_t NumOfExtensions)
{
    ...
    do {
    	...
    	if (!DoEDID_Checksum(g_CommData)) {
    		TX_EDID_PRINT("EDID(%d) -> Checksum Error\n",(int)Block);
    		return EDID_CHECKSUM_ERROR;
    	}
    ...
    while (DataIndex < LongDescriptorOffset) {
        ...
        switch (TagCode) {
            ...
            case VENDOR_SPEC_D_BLOCK:
                ...
                /**
                 * original get cec address session
                 */
...
uint8_t Parse861LongDescriptors(uint8_t *Data)
{
    ...
    if (!LongDescriptorsOffset) {
        TX_EDID_PRINT("EDID -> No Detailed Descriptors\n");
